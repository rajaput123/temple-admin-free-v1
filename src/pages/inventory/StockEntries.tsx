...
import { dummyCounters } from '@/data/temple-structure-data';
...
export default function StockEntries() {
  ...
  const getItemName = (itemId: string) => {
    return dummyItems.find(i => i.id === itemId)?.name || 'Unknown';
  };
  ...
  const entryColumns = [
    { key: 'entryNumber', label: 'Entry Number', sortable: true },
    {
      ...
      render: (value: unknown) => (
        <Badge variant={value === 'receipt' ? 'default' : value === 'issue' ? 'secondary' : 'outline'}>
          {entryTypeLabels[value as StockEntryType]}
        </Badge>
      ),
    },
  ...
  return (
    <MainLayout>
      <PageHeader
        title="Stock Entries"
        description="Record stock movements, receipts, issues, and inventory transactions"
        actions={
